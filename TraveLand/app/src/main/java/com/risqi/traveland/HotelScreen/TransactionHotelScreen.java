...
public class TransactionHotelScreen extends AppCompatActivity {
    ...
    private TransactionHotel transactionHotel ;
    ...
    private TextView tetxtPemesanan, jNamaHotel,tNamaHotel,jAlamatHotel,tAlamatHotel,jKamar,tKamar,jHarga,tHarga,jumlahKamar,jCheckin,jCheckout,tempDate;
    ...
    private TransactionHotelScreen.PaymentRecyclerViewAdapter paymentRecyclerViewAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_transaction_hotel_screen);
        getIntents();
        initialize();
    ...
    private void pesanNow() {
        PesanSekarang.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                new SweetAlertDialog(TransactionHotelScreen.this, SweetAlertDialog.WARNING_TYPE)
                                ...
                                new Handler().postDelayed(new Runnable() {
                                    ...
                                    public void run() {
                                        if(keyPayments.equals("")){
                                            pDialog.dismiss();
                                            new SweetAlertDialog(TransactionHotelScreen.this, SweetAlertDialog.ERROR_TYPE)
                                                    .setTitleText("Oops...")
    ...
    private void sendTranksaksi() {
        int value = Integer.parseInt(jumlahKamar.getText().toString());
        ...
        HashMap updateDetailHotel = masterDataHotelDetail.updateJumlahKamar(""+(jumlahKamarr-Integer.parseInt(jumlahKamar.getText().toString())));
        updateData1 = FirebaseDatabase.getInstance().getReference().child("Master-Data-Hotel-Detail").child(idDetail).updateChildren(updateDetailHotel);
        updateData1.addOnCompleteListener(new OnCompleteListener() {
            @Override
            public void onComplete(@NonNull Task task) {
                Map<String, String> insertData = transactionHotel.insertData(NIKUSER,idHotel,idDetail,hargakamarr+"",jumlahKamar.getText().toString(),tCheckin.getText().toString(),tCheckOut.getText().toString(),keyPayments,""+selisihHari,""+totalJUmlah);
                database1 = FirebaseDatabase.getInstance().getReference().child("Transaction-Hotel").child(key);
                database1.setValue(insertData).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        Intent a = new Intent(TransactionHotelScreen.this, NotifTransactionSuccessScreen.class);
                        a.putExtra("Next", "UploadFotoHotel");
                        a.putExtra("idScreen", ""+key);
                        startActivity(a);
                        Animatoo.animateSlideLeft(TransactionHotelScreen.this);
                        onStop();
                    }
                });
            }
    ...
    private void SetTombolPlusMINUS() {
        PlusKamar.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                int value = Integer.parseInt(jumlahKamar.getText().toString());
                if(value<jumlahKamarr){
                    jumlahKamar.setText(""+(value+1));
                    Date date1 = ConvertStringToDate(tCheckin.getText().toString(), polaCOnvert, null);
                    Date date2 = ConvertStringToDate(tCheckOut.getText().toString(), polaCOnvert, null);
                    long tempDataDate = Math.abs(date2.getTime() - date1.getTime());
                    long selisihHari = tempDataDate / (24 * 60 * 60 * 1000);
                    totalDewasa.setText("Rp."+(hargakamarr*((value+1)*selisihHari)));
                    totalPembayaran.setText("Rp."+(hargakamarr*((value+1)*selisihHari)));
                }
            }
        });
    ...
    private void SetTanggalout() {
        ...
        tCheckOut.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                DatePickerDialog datePickerDialogs = new DatePickerDialog(TransactionHotelScreen.this, date, calendarTemp.get(Calendar.YEAR), calendarTemp.get(Calendar.MONTH), calendarTemp.get(Calendar.DAY_OF_MONTH));
                ...
                nowDate2.add(Calendar.DATE,Integer.parseInt(tempDate.getText().toString())-1);
...
//                datePickerDialogs.getDatePicker().set(myCalendar.getTimeInMillis()+10);
    ...
    private void SetTanggalIN() {
        ...
        tCheckin.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                DatePickerDialog datePickerDialogs = new DatePickerDialog(TransactionHotelScreen.this, date, myCalendar.get(Calendar.YEAR), myCalendar.get(Calendar.MONTH), myCalendar.get(Calendar.DAY_OF_MONTH));
...
//            Toast.makeText(this, "" + mod.getString(mod.getColumnIndexOrThrow("mode")), Toast.LENGTH_SHORT).show();
        ...
        database2.child("Master-Data-Hotel").child(idHotel).get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                Map<String, Object> hotel = (Map<String, Object>) task.getResult().getValue();
                tNamaHotel.setText(wordCase(hotel.get("NamaHotel").toString()));
                tAlamatHotel.setText(wordCase(hotel.get("AlamatHotel").toString()));
            }
        });

        database3= FirebaseDatabase.getInstance().getReference();
        database3.child("Master-Data-Hotel-Detail").child(idDetail).get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                Map<String, Object> Detailhotel = (Map<String, Object>) task.getResult().getValue();
                tKamar.setText(wordCase(Detailhotel.get("NamaKamar").toString()));
                tempDate.setText(wordCase(Detailhotel.get("MaksimalMenginap").toString()));
                tHarga.setText("Rp."+wordCase(Detailhotel.get("HargaKamar").toString()));
                jumlahKamarr = Integer.parseInt(Detailhotel.get("JumlahKamar").toString());
                hargakamarr = Integer.parseInt(Detailhotel.get("HargaKamar").toString());
                totalTextDewasa.setText("Total Sewa ("+1+" Hari)");
                totalDewasa.setText("Rp."+hargakamarr);
                totalPembayaran.setText("Rp."+hargakamarr);
    ...
    private void toMenu() {
        btnbackscroller.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent a = new Intent(TransactionHotelScreen.this, DetailHotelScreen.class);
    ...
    private void setDataPayment() {
        paymentRecyclerViewAdapter = new TransactionHotelScreen.PaymentRecyclerViewAdapter(TransactionHotelScreen.this, masterDataBanks, listPayment);
...
//            Toast.makeText(this, "" + mod.getString(mod.getColumnIndexOrThrow("mode")), Toast.LENGTH_SHORT).show();
        ...
        if (modeApps.equals("Malam")) {
            ...
            jumlahKamar.setTextColor(getResources().getColor(R.color.white));
            jNamaHotel.setTextColor(getResources().getColor(R.color.white));
            tNamaHotel.setTextColor(getResources().getColor(R.color.white));
            jAlamatHotel.setTextColor(getResources().getColor(R.color.white));
            tAlamatHotel.setTextColor(getResources().getColor(R.color.white));
    ...
    private void initialize() {
        ...
        //Sweet Alert
        pDialog = new SweetAlertDialog(TransactionHotelScreen.this, SweetAlertDialog.PROGRESS_TYPE);
...
//        Layout Detail Wisata
        ...
        PlusKamar = findViewById(R.id.PlusDewasa);
        jumlahKamar = findViewById(R.id.jumlahDewasa);
        minusKamar = findViewById(R.id.minusDewasa);
        bgHotel = findViewById(R.id.bgWisata);
        bgCheckin = findViewById(R.id.bgChechin);
        bgCheckout = findViewById(R.id.bgChechout);
        jNamaHotel = findViewById(R.id.judulWisata);
        tNamaHotel = findViewById(R.id.NamaWisata);
        jAlamatHotel = findViewById(R.id.JudulAlamatWisata);
        tAlamatHotel = findViewById(R.id.AlamatWisata);
    ...
    private class PaymentRecyclerViewAdapter extends RecyclerView.Adapter<TransactionHotelScreen.PaymentRecyclerViewAdapter.NameViewHolder> {
        ...
        @NonNull
        @Override
        public TransactionHotelScreen.PaymentRecyclerViewAdapter.NameViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            LayoutInflater inflater = LayoutInflater.from(parent.getContext());
            View v = inflater.inflate(R.layout.item_list_payment, parent, false);
            return new TransactionHotelScreen.PaymentRecyclerViewAdapter.NameViewHolder(v);
        }

        @Override
        public void onBindViewHolder(@NonNull TransactionHotelScreen.PaymentRecyclerViewAdapter.NameViewHolder holder, @SuppressLint("RecyclerView") int position) {
...
//            Toast.makeText(this, "" + mod.getString(mod.getColumnIndexOrThrow("mode")), Toast.LENGTH_SHORT).show();
            ...
            if(databbanks.getGambarBank().equals("")){
                ...
                Glide.with(TransactionHotelScreen.this).clear(holder.imageBank);
                Glide.with(TransactionHotelScreen.this)
