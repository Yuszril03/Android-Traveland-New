...
public class KomentarRentalRecyclerViewAdapter extends RecyclerView.Adapter<KomentarRentalRecyclerViewAdapter.NameViewHolder> {
    ...
    @NonNull
    @Override
    public KomentarRentalRecyclerViewAdapter.NameViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        LayoutInflater inflater = LayoutInflater.from(parent.getContext());
        View v = inflater.inflate(R.layout.item_list_komentar, parent, false);
        return new KomentarRentalRecyclerViewAdapter.NameViewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull KomentarRentalRecyclerViewAdapter.NameViewHolder holder, int position) {

        TransactionRental transactionRental = datakomentar.get(position);
...
//            Toast.makeText(this, "" + mod.getString(mod.getColumnIndexOrThrow("mode")), Toast.LENGTH_SHORT).show();
        ...
        database1.child("Master-Data-Customer").child(transactionRental.getIdCutomer()).get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
...
//                    .transform(new MultiTransformation(new FitCenter()))
        ...
        int rate =0;
        if(!transactionRental.getRating().equals("")){
            rate = Integer.parseInt(transactionRental.getRating());
        }
        ...
        if(transactionRental.getUlasanCustomer().equals("")){
            ...
            holder.komentarUser.setText(transactionRental.getUlasanCustomer().toString());
        }
        holder.KomentarMitra.setText(transactionRental.getUlasanMitra().toString());
        if(transactionRental.getUlasanMitra().equals("")){
//            holder.bgkomeMitra.setMaxHeight(0);
    ...
    private List<TransactionRental> datakomentar;
    ...
    public KomentarRentalRecyclerViewAdapter(Context context, List<TransactionRental> datakomentar) {
