...
public class HotelRecyclerViewAdapter extends RecyclerView.Adapter<HotelRecyclerViewAdapter.NameViewHolder> {
    ...
    @Override
    public void onBindViewHolder(@NonNull HotelRecyclerViewAdapter.NameViewHolder holder, @SuppressLint("RecyclerView") int position) {
...
//            holder. imgChild.setBackgroundResource(R.drawable.icon_person_dark);
        ...
        getReference.child("Master-Data-Hotel").child(masterdatahoteldetail.getIdHotel()).get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                ...
                String textTemp = hotel.get("AlamatHotel").toString();

                String judul = masterdatahoteldetail.getNamaKamar() + " - " + hotel.get("NamaHotel");
                if (judul.length() < 23) {
                    holder.JudulWisata.setText(wordCase(masterdatahoteldetail.getNamaKamar() + " - " + hotel.get("NamaHotel")));
                    if (textTemp.length() <= 55) {
                        holder.AlamatWisata.setText("" + wordCase(hotel.get("AlamatHotel").toString()));
                    } else {
                        String resultAlamat = "";
...
//                            .override(135, 135)
    ...
    public int getItemCount() {
        return masterDataHotelDetail.size();
    }
